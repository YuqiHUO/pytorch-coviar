...
void create_and_load_mv_residual_b(
    AVFrameSideData *sd, 
    PyArrayObject * bgr_arr,
    PyArrayObject * mv_arr,
    PyArrayObject * res_arr,
...
int decode_video(
    ...
    while (1) {
        ...
        while (cur_size) {  
            ...
            if(!packet.size)//data is incomplete, cont generate one frame
                continue;
            ...
            if (cur_gop == gop_target || (cur_gop <= gop_target + 1 && cur_pos == 0)) { //every I frame must be read
                if (!(cur_pos == pos_target && cur_gop == gop_target)) {
					if (pCodecParserCtx -> pict_type == AV_PICTURE_TYPE_B) {
						cur_pos++;
						continue;
					}
					if (flag)
                ...
                if ((representation == MV ||
                    ...
                    b_accu_src     = (int*) malloc(w * h * 2 * sizeof(int));
                    b_accu_src_old = (int*) malloc(w * h * 2 * sizeof(int));
...
printf("%d %d %d %d %d %d %d %d\n", ret, h, w, got_picture, cur_gop, cur_pos, flag, flag);
                if (got_picture) {
                    printf("got picture\n");
                    create_and_load_bgr(pFrame, pFrameBGR, buffer, bgr_arr, cur_pos, pos_target);////这里得到的数据，无论IPB帧都是当前解压之后的画面，因为这是API提供的默认功能。所以更要避免
                    
					if (flag && !end_flag) {// first time in target B frame
                        if (!representation)
                            break;
                        if (!accumulate) {
                            AVFrameSideData *sd2;
							sd2 = av_frame_get_side_data(pFrame, AV_FRAME_DATA_MOTION_VECTORS);
							if (sd2){ 
                            ...
                            sdb = av_frame_get_side_data(pFrame, AV_FRAME_DATA_MOTION_VECTORS); 
                        }
					} else if (flag && end_flag) {// second time in key frame (I/P) after the target B frame and *accumulate*
                    //b_ is previous frame and the other is the latter frame
						if (!representation)
                            break;
                        memcpy(b_accu_src_old, accu_src_old, h * w * 2 * sizeof(int)); 
                        memcpy(b_accu_src, accu_src, h * w * 2 * sizeof(int)); 
                        AVFrameSideData *sd3;
                        sd3 = av_frame_get_side_data(pFrame, AV_FRAME_DATA_MOTION_VECTORS);
                        if (sd3){
                            create_and_load_mv_residual(
                            sd3, 
                            *bgr_arr, *mv_arr, *res_arr,
                        ...
                        for (size_t x = 0; x < w; ++x) {
                            for (size_t y = 0; y < h; ++y) {
                                b_accu_src[x * h * 2 + y * 2    ]  = (accu_src[x * h * 2 + y * 2    ] + b_accu_src[x * h * 2 + y * 2    ]) / 2;
                                b_accu_src[x * h * 2 + y * 2 + 1]  = (accu_src[x * h * 2 + y * 2 + 1] + b_accu_src[x * h * 2 + y * 2 + 1]) / 2;
                                b_accu_src_old[x * h * 2 + y * 2    ]  = (accu_src_old[x * h * 2 + y * 2    ] + b_accu_src_old[x * h * 2 + y * 2    ]) / 2;
                                b_accu_src_old[x * h * 2 + y * 2 + 1]  = (accu_src_old[x * h * 2 + y * 2 + 1] + b_accu_src_old[x * h * 2 + y * 2 + 1]) / 2;
                            }
                        }
                        create_and_load_mv_residual_b(sdb, *bgr_arr, *mv_arr, *res_arr, representation, b_accu_src, b_accu_src_old, w, h);
                        free(sdb);
					} else {
                        printf("im here\n");
						if (representation) {
                            AVFrameSideData *sd4;
							sd4 = av_frame_get_side_data(pFrame, AV_FRAME_DATA_MOTION_VECTORS);
							if (sd4) {// cur_pos is not I frame
